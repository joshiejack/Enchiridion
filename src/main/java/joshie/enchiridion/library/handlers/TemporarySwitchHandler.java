...
public class TemporarySwitchHandler implements IBookHandler {
    ...
    public void handle(ItemStack stack, EntityPlayer player, EnumHand hand, int slotID, boolean isShiftPressed) {
        try {
            ...
            if (HeldHelper.getStackFromHand(player, hand) != null) held = HeldHelper.getStackFromHand(player, hand).copy(); //Store the held item
            player.setItemStackToSlot(slot, stack.copy()); //Replace the held item with the item in the book
            ItemStack ret = player.getActiveItemStack().useItemRightClick(player.worldObj, player, hand).getResult(); //Grab the result of right clicking
